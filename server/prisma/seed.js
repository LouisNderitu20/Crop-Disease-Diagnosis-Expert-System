    ...
    for (const r of knowledgeBaseData.rules) {
        ...
        if (!disease) {
            disease = await prisma.disease.create({
                data: {
                    name: r.then.disease,
                    treatment: JSON.stringify(r.then.treatment),
                    prevention: JSON.stringify(r.then.prevention)
                }
            });
        }
        ...
        await prisma.rule.create({
            data: {
                ruleId: r.id,
                cropId: crop.id,
                conditionSymptoms: JSON.stringify(r.condition.symptoms),
                conditionEnv: JSON.stringify(r.condition.environmental || []),
                diseaseId: disease.id,
                confidence: r.then.confidence
            }
        });
    }
